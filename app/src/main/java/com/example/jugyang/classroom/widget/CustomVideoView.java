...
import static com.example.jugyang.classroom.utils.StaticClass.VIDEO_SCREEN_PERCENT;

/**
 * Project Name:     Classroom_Toy_v1.0
 * Package Name:     com.example.jugyang.classroom.widget
 * File Name:        CustomVideoView
 * Description:      Created by jugyang on 4/20/17.
 * Function:         负责视频播放,暂停以及各类事件的触发
 */
...
public class CustomVideoView extends RelativeLayout implements View.OnClickListener,
    ...
    private void initData() {
        ...
        mDestationHeight = (int) (mScreenWidth * StaticClass.VIDEO_HEIGHT_PERCENT);
    }
    ...
    protected void onVisibilityChanged(View changedView, int visibility) {
        MyLog.e(TAG + "onVisibilityChanged" + visibility);
        ...
        if (v == this.mMiniPlayBtn) {
            if (this.playerState == STATE_PAUSING) {
                if (Utils.getVisiblePercent(mParentContainer)
                        > StaticClass.VIDEO_SCREEN_PERCENT) {
                    resume();
    ...
    public void onCompletion(MediaPlayer mp) {
        if (listener != null) {
            listener.onAdVideoLoadComplete(); //通知外部当前视频播放完成
        }
        playBack(); //回到初始状态
        setIsComplete(true);
        setIsRealPause(true);
    ...
    public void onPrepared(MediaPlayer mp) {
        ...
        if (mediaPlayer != null) {
            ...
            mCurrentCount = 0;
            if (listener != null) {
                listener.onAdVideoLoadSuccess(); //通知外部视频加载成功
            }
            //满足自动播放条件，则直接播放
            if (Utils.canAutoPlay(getContext(),
                    AdParameters.getCurrentSetting()) &&
                    ( Utils.getVisiblePercent(mParentContainer) > StaticClass.VIDEO_SCREEN_PERCENT)) {
    ...
    public void load() {
        ...
        MyLog.d(TAG + "do play url = " + this.mUrl);
        showLoadingView();
        try {
            setCurrentPlayState(STATE_IDLE);
            checkMediaPlayer(); //完成播放器创建工作
            mute(true);
        ...
        if (mediaPlayer != null) {
            ...
            mediaPlayer.setOnSeekCompleteListener(new MediaPlayer.OnSeekCompleteListener() {
                ...
                public void onSeekComplete(MediaPlayer mp) {
                    MyLog.d(TAG + "do seek and resume");
                    mediaPlayer.start();
    ...
    public void stop() {
        ...
        mHandler.removeCallbacksAndMessages(null); //对外通知视频播放到了第几秒
        setCurrentPlayState(STATE_IDLE);
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
        MyLog.i(TAG + "onSurfaceTextureAvailable");
    ...
    public void onSurfaceTextureSizeChanged(SurfaceTexture surface, int width, int height) {
        MyLog.i(TAG + "onSurfaceTextureSizeChanged");
    }
    ...
    private void decideCanPlay() {
        if (Utils.getVisiblePercent(mParentContainer) > StaticClass.VIDEO_SCREEN_PERCENT)
